...
% 
% Use as
%   >> noninformativetask(subj,eeg,tc,runt)
%
% Input Arguments
...
try
    ...
    % load images
    instructions    = imread('instruction_invalid.tif');
    ...
    while left_unseen < 3 || left_unseen > 7 || right_unseen < 3 || right_unseen > 7 % while the percentage of unseen trials is a small or big
        ...
        load noninformativetask_aleatorization
        ...
        for trl=1:Nptrials
            ...
            % grab gabor presence/location (0 = left, 1 = right, 9 = no gabor)
            ...
            elseif ix == 0 % Gabor presented in the left hemifield
                ...
                Screen('DrawTexture',w,txt,[0 0 1000 1000],[coord(nix,1) coord(nix,2) coord(nix,3)  coord(nix,4)]);

            elseif ix == 1 % Gabor presented in the right hemifield
            ...
            % photodiode
            if tc == 1
                Screen('DrawTexture',w,txt_white,[0 0 4000 2250],[1725*x0*2/1920 850*y0*2/1080 x0*2+1 y0*2]); %gris claro
            end
            Screen ('Flip',w);
            KbWait
            ...
            nresp = round(1 + (4-1).*rand(1)) ;   % number of repeated resps
            if ix == 0 % gabor in left hemifield
            ...
            elseif ix == 1 % gabor in right hemifield
        ...
        left_unseen  = (Nptrials./2)-sum(left_resp);
        right_unseen = (Nptrials./2)-sum(right_resp);
        
         % keep adjusting initial Gabor contrast range 
        if left_unseen > 7 % if more than 7 trials are seen in left hemifield, reduce range of that hemifield
        ...
        elseif left_unseen < 3 % if less than 3 trials are unseen in left hemifield, increase range of that hemifield
        ...
        elseif right_unseen > 7 % if more than 7 trials are seen in right hemifield, reduce range of that hemifield
        ...
        elseif right_unseen < 3 % if less than 3 trials are unseen in right hemifield, increase range of that hemifield
    ...
    left_thr  = left_thr(left_unseen);
    right_thr = sortrows(right_threshold);
    right_thr = right_thr(right_unseen);
    
    % calibration parameters
    sigma = 0.001;     % stimulus variability around threshold (reduced)
    thr_step = 0.001;  % step size (reduced)
    
    % announces the start of the experimental block    Screen ('FillRect',w,127);
    Screen('TextFont', w , 'Helvetica')
    Screen('TextSize', w, 25);
    ...
    elseif runt == 1 % run whole task
        for trl = 1:Ntrial
            ...
            if parameters(trl,4) > 100 && parameters(trl,4) < 103 && trigger(trl,3) ==124 &&...
               ct_leftseen > 0 && rand(1) < 0.15 || parameters(trl,4) > 100 && ...
               parameters(trl,4) < 103 && trigger(trl,3) == 125 && ct_leftunseen > 0 &&...
               rand(1) < 0.15 || parameters(trl,4) > 112 && parameters(trl,4) < 115 && ...
               trigger(trl,3) == 124 && ct_rightseen > 0 && rand(1) < 0.15 || parameters(trl,4) > 112 &&...
               parameters(trl,4) < 115 && trigger(trl,3) == 125 && ct_rightunseen > 0 &&...
               rand(1) < 0.15 || parameters(trl,4) > 110 && parameters(trl,4) < 113 &&...
               trigger(trl,3) == 124 && ct_inleftseen > 0 && rand(1) < 0.15 || parameters(trl,4) > 110 &&...
               parameters(trl,4) < 113 && trigger(trl,3) == 125 && ct_inleftunseen > 0 &&...
               rand(1) < 0.15 || parameters(trl,4) > 102 && parameters(trl,4) < 105 &&...
               trigger(trl,3) == 124 && ct_inrightseen > 0 && rand(1) < 0.15 || parameters(trl,4) > 102 &&...
               parameters(trl,4) < 105 && trigger(trl,3) == 125 && ct_inrightunseen > 0 && rand(1) < 0.15
                ...
                while time < 0.1
                    ...
                    if (keyIsDown) && cont_r == 0
                        ...
                        if parameters(trl,4) == 101 && trg == 121 || parameters(trl,4) == 111 && trg == 121 ||...
                           parameters(trl,4) == 103 && trg == 121 || parameters(trl,4) == 113 && trg == 121 ||...
                           parameters(trl,4) == 102 && trg == 122 || parameters(trl,4) == 112 && trg == 122 ||...
                           parameters(trl,4) == 104 && trg == 122 || parameters(trl,4) == 114 && trg == 122
                ...
                % update counters for objective question
                    %valid trials
                if parameters(trl,4) > 100 &&  parameters(trl,4) < 103 && trigger(trl,3) == 125 %left_unseen
                    ct_leftunseen = ct_leftunseen-1;
                elseif parameters(trl,4) > 100 &&  parameters(trl,4) < 103 && trigger(trl,3) == 124 %left_seen
                    ct_leftseen = ct_leftseen-1;
                elseif parameters(trl,4) > 112 &&  parameters(trl,4) < 115 && trigger(trl,3) == 125 %right_unseen
                    ct_rightunseen = ct_rightunseen-1;
                elseif parameters(trl,4) > 112 &&  parameters(trl,4) < 115 && trigger(trl,3) == 124 %right_seen
                    ct_rightseen = ct_rightseen-1;
                    
                    %invalid trials
                elseif parameters(trl,4) > 110 &&  parameters(trl,4) < 113 && trigger(trl,3) == 125 %left_unseen
                    ct_inleftunseen = ct_inleftunseen-1;
                elseif parameters(trl,4) > 110 &&  parameters(trl,4) < 113 && trigger(trl,3) == 124 %left_seen
                    ct_inleftseen = ct_inleftseen-1;
                elseif parameters(trl,4) > 102 &&  parameters(trl,4) < 105 && trigger(trl,3) == 125 %right_unseen
                    ct_inrightunseen = ct_inrightunseen-1;
                elseif parameters(trl,4) > 102 &&  parameters(trl,4) < 105 && trigger(trl,3) == 124 %right_seen
                    ct_inrightseen = ct_inrightseen-1;
                end
            end
            ...
            if trl == 400 % if 400 trials, experiment is done
                ...
                % save variables
                save invalid_responses discrimination_response discrimination_time response_key right_resp left_resp response_time trigger right_threshold right_stim_contrast left_threshold left_stim_contrast
