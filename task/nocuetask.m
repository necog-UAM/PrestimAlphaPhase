%
% nocuetask() - this function uses Psychtoolbox to control stimulus 
%             presentation and behavioral data collection during one of the  
%             near-threshold visual detection tasks used in Melcón at al.  
%             2004, specifically, an no-cue task.  
%             Each trial starts with a variable fixation window, where two  
%             boxes outlined in light gray are placed at the lower left and  
%             right sides of a central cross. A Gabor stimulus is presented 
%             at peri-threshold contrast during 50ms, whose orientation is 
%             equally likely to be either vertical or horizontal (no 
%             stimulus is presented in 10% of trials). Participants are 
%             asked about two aspects related to the Gabor. First, a 
%             subjective question: Have you seen the Gabor stimulus? 
%             Response: yes or no. Then, an objective question in 15% of 
%             randomly selected trials: What was the Gabor stimulus 
%             orientation? Response: horizontal or vertical. In both 
%             question displays the two response alternatives (yes/no, 
%             horizontal/vertical) appeared randomly on the right and the  
%             left. Responses are collected from a numerical keyboard  
%             during unlimited time. The stimulus contrast is calculated on
...
function nocuetask(subj,eeg,tc,runt)
...
try
    ...
    trigger                 = nan(Ntrial,3); % columns: cue - gabor - subjective task response - objective task response
    ...
    size_cue1 = [0 0 162 162];%[0 0 152 152];
    size_cue2 = [0 0 150 150];%[0 0 140 140];

    % load images
    instruction     = imread('instruction_perception.tif');
    ...
    % instructions
    txt = Screen('MakeTexture',w,instruction);
    ...
    while left_unseen < 3 || left_unseen > 7 || right_unseen < 3 || right_unseen > 7% while the percentage of unseen trials is a small or big
        ...
        load nocuetask_aleatorization
        
        for trl = 1:Nptrials % start trial presentation for pre-task calibration block
            ...
            elseif ix == 1 % Gabor presented in the left hemifield
                ...
                if rem(parameters(trl,2),2) > 0 % Vertically orientated
                    numb_img = (fx_Gabor_patch_A(lstim_tr,0)+1).*127;
                elseif rem(parameters(trl,2),2) == 0 % Horizontally oriented
            ...
            % 4. Subjective task (Have you seen the Gabor? Yes/No)
            ...
            while time < 0.3
                ...
                if (keyIsDown) && cont_r == 0 
                    ...
                    if ans_scr > 0.5 % accordingly with Yes/No location on the screen
                        if strcmp(key,'4')
                            trigger(trl,2) = 124; %seen
                            trg = 124;
                        elseif strcmp(key,'6')
                            trigger(trl,2) = 125; %unseen
                            trg = 125;
                        end
                    else
                        if strcmp(key,'4')
            ...
            if ix == 1 % gabor in left hemifield
                ...
                left_threshold (length(left_resp),1) = left_thr(ct_l);
                left_stim_contrast(length(left_resp),1) = lstim_tr;
                ct_l=ct_l+1;
                
            elseif ix == 2 % gabor in right hemifield
                right_resp=[right_resp resp];
                right_threshold (length(right_resp),1) = right_thr(ct_r);
                right_stim_contrast(length(right_resp),1) = rstim_tr;
                ct_r=ct_r+1;
            end
    ...
    elseif runt == 1
        for trl = 1:Ntrial
            ...
            elseif ix == 1 % Gabor presented in the left hemifield
                ...
                if rem(parameters(trl,2),2) > 0 % Vertically oriented
                    numb_img = (fx_Gabor_patch_A(lstim_tr,0)+1).*127;
                elseif rem(parameters(trl,2),2) == 0 % Horizontally oriented
                    numb_img = (fx_Gabor_patch_A(lstim_tr,90)+1).*127;
                end
            ...
            % 5. Objective task - 15% (What was the Gabor orientation? Vertical/Horizontal)   
            if parameters(trl,2) > 10 && parameters(trl,2) < 13 && trigger(trl,2) == 124 &&...
               ct_leftseen > 0 && rand(1) < 0.15 || parameters(trl,2) > 10 && parameters(trl,2) < 13 &&...
               trigger(trl,2) == 125 && ct_leftunseen > 0 && rand(1) < 0.15 || parameters(trl,2) > 20 &&...
               parameters(trl,2) < 23 && trigger(trl,2) == 124 && ct_rightseen > 0 &&...
               rand(1) < 0.15 || parameters(trl,2) > 20 && parameters(trl,2) < 23 &&...
               trigger(trl,2) == 125 && ct_rightunseen > 0 && rand(1) < 0.15
                ...
                while time < 0.1
                    ...
                    if (keyIsDown) && cont_r==0
                        ...
                        if disc_scr > 0.5 % accordingly with Vertical/Horizontal location on the screen
                            if strcmp(key,'4')
                                trg = 121;
                                trigger(trl,3) = 121; %vertical
                                
                            elseif strcmp(key,'6')
                                trigger(trl,3) = 122; %horizontal
                                trg = 122;
                            end
                        else
                            if strcmp(key,'4')
                        ...
                        discrimination_response(trl) = trg;
                        if parameters(trl,2) == 11 && trg == 121 || parameters(trl,2) == 21 &&...
                           trg == 121 || parameters(trl,2) == 12 && trg== 122 ||...
                           parameters(trl,2) == 22 && trg == 122
                ...
                if parameters(trl,2) > 10 &&  parameters(trl,2) < 13 && trigger(trl,2) == 125 %left_unseen
                    ct_leftunseen = ct_leftunseen-1;
                elseif parameters(trl,2) > 10 &&  parameters(trl,2) < 13 && trigger(trl,2) == 124 %left_seen
                    ct_leftseen = ct_leftseen-1;
                elseif parameters(trl,2) > 20 &&  parameters(trl,2) < 23 && trigger(trl,2) == 125 %right_unseen
                    ct_rightunseen = ct_rightunseen-1;
                elseif parameters(trl,2) > 20 &&  parameters(trl,2) < 23 && trigger(trl,2) == 124 %right_seen
            ...
            if trl == 400 % if 400 trials, experiment is done
                ...
                % save variables
                save perception_responses discrimination_response discrimination_time response_key right_resp left_resp response_time trigger right_threshold right_stim_contrast left_threshold left_stim_contrast
